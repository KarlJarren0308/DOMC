...
class MainController extends Controller
{
    ...
    public function postSearchOpac(Request $request) {
        ...
        if($request->input('searchBy') == 'Author') {
            $data['works_materials'] = Works::where('authors.Author_First_Name', 'like', '%' . $request->input('searchKeyword') . '%')->orWhere('authors.Author_Middle_Name', 'like', '%' . $request->input('searchKeyword') . '%')->orWhere('authors.Author_Last_Name', 'like', '%' . $request->input('searchKeyword') . '%')->orWhereRaw("CONCAT(authors.Author_First_Name, ' ', authors.Author_Last_Name) LIKE '%" . $request->input('searchKeyword') . "%'")->orWhereRaw("CONCAT(authors.Author_First_Name, ' ', authors.Author_Middle_Name, ' ', authors.Author_Last_Name) LIKE '%" . $request->input('searchKeyword') . "%'")->join('materials', 'works.Material_ID', '=', 'materials.Material_ID')->leftJoin('authors', 'works.Author_ID', '=', 'authors.Author_ID')->groupBy('works.Material_ID')->get();
        } else {
            $data['works_materials'] = Works::where('materials.' . $request->input('searchBy'), 'like', '%' . $request->input('searchKeyword') . '%')->join('materials', 'works.Material_ID', '=', 'materials.Material_ID')->leftJoin('authors', 'works.Author_ID', '=', 'authors.Author_ID')->groupBy('works.Material_ID')->get();
        }
